...
> ⚠️ **注意**：文档中所有行号引用（如 `NrrdTools.ts:173`、`CommToolsData.ts:351` 等）均来自历史版本，经过多轮重构（State Management Refactor + NrrdTools God Class Split）后已过时，仅作结构参考，请以实际代码为准。
...
CommToolsData          ← 基类：Canvas 管理、状态初始化、渲染管线
  └── DrawToolCore     ← 事件编排（orchestration）、Undo/Redo、Tool 初始化与委托
       └── NrrdTools   ← 对外 API Facade（1300 行，13 个分区）
...
>
> **NrrdTools God Class Split（已完成）**: NrrdTools 经过 4 个 Phase 的重构，从 2007 行 God Class 拆分为 Facade + 3 个功能模块。公开 API 不变，内部通过 `ToolContext` + `*Callbacks` 模式解耦。
...
- [NrrdTools.ts](annotator-frontend/src/ts/Utils/segmentation/NrrdTools.ts) — 对外 API Facade（13 分区）
...
NrrdTools 通过 `ToolContext` 将共享状态传递给各模块，通过 `*Callbacks` 接口回调宿主方法：
...
| 模块 | 文件 | 职责 | Callbacks |
|------|------|------|-----------|
| **LayerChannelManager** | `tools/LayerChannelManager.ts` | setActiveLayer/Channel/SphereType、可见性控制、自定义通道颜色 | 3 个 (reloadMasksFromVolume, getVolumeForLayer, onChannelColorChanged) |
| **SliceRenderPipeline** | `tools/SliceRenderPipeline.ts` | 切片轴配置、canvas 渲染、mask 重载、canvas 翻转、view/canvas 辅助 | 10 个 |
| **DataLoader** | `tools/DataLoader.ts` | NRRD 切片加载、legacy mask 加载、NIfTI voxel 加载 | 7 个 (invalidateSliceBuffer, setDisplaySlicesBaseOnAxis, afterLoadSlice, ...) |
...
**接口**: `INrrdStates` extends `IImageMetadata`, `IViewState`, `IInteractionState`, `ISphereState`, `IInternalFlags`（定义: [coreTools/coreType.ts](annotator-frontend/src/ts/Utils/segmentation/coreTools/coreType.ts)）
...
**接口**: `IGUIStates` extends `IToolModeState`, `IDrawingConfig`, `IViewConfig`, `ILayerChannelState`（定义: [coreTools/coreType.ts](annotator-frontend/src/ts/Utils/segmentation/coreTools/coreType.ts)）
...
定义位置: [CommToolsData.ts](annotator-frontend/src/ts/Utils/segmentation/CommToolsData.ts)
...
存储位置: `CommToolsData.annotationCallbacks.onLayerVolumeCleared`
...
存储位置: `CommToolsData.annotationCallbacks.onChannelColorChanged`（`IAnnotationCallbacks` 接口，`coreType.ts`）

> ⚠️ **注意**: 原文档说定义在 `INrrdStates` 上，**已不正确**。该回调现在属于 `IAnnotationCallbacks`，存储在 `CommToolsData.annotationCallbacks` 中，不在 `nrrd_states` 上。
...
存储位置: `CommToolsData.annotationCallbacks`（`IAnnotationCallbacks`，外部通过 `draw()` 注册）
...
**重要**: `renderSliceToCanvas()`（CommToolsData.ts）中 Mask 渲染**对冠状面（axis='y'）做垂直翻转**：

```ts
// CommToolsData.renderSliceToCanvas() — Y 轴翻转：
if (axis === 'y') {
  targetCtx.save();
  targetCtx.scale(1, -1);
...
`CommToolsData.applyMaskFlipForAxis()` — 提供与 `flipDisplayImageByAxis()` 相同的翻转变换，可供需要手动对齐坐标的场景使用。目前在主渲染路径中的具体冠状面翻转已内联到 `renderSliceToCanvas()`。
...
这三个模块由 `NrrdTools.initNrrdToolsModules()` 初始化，通过 `ToolContext` + `*Callbacks` 模式与 NrrdTools 解耦：

| 模块 | 文件 | 行数 | 说明 |
|------|------|------|------|
| **LayerChannelManager** | [tools/LayerChannelManager.ts](annotator-frontend/src/ts/Utils/segmentation/tools/LayerChannelManager.ts) | 211 | 图层/通道/SphereType 管理，可见性控制，自定义通道颜色。3 个 callbacks: `reloadMasksFromVolume`, `getVolumeForLayer`, `onChannelColorChanged` |
| **SliceRenderPipeline** | [tools/SliceRenderPipeline.ts](annotator-frontend/src/ts/Utils/segmentation/tools/SliceRenderPipeline.ts) | 453 | 切片轴配置（`setDisplaySlicesBaseOnAxis`）、canvas 渲染（`reloadMasksFromVolume`、`redrawDisplayCanvas`、`flipDisplayImageByAxis`）、view 辅助（`resizePaintArea`、`resetPaintAreaUIPosition`）。10 个 callbacks |
| **DataLoader** | [tools/DataLoader.ts](annotator-frontend/src/ts/Utils/segmentation/tools/DataLoader.ts) | 222 | NRRD 切片加载（`setAllSlices`）、legacy mask 加载（`setMasksData`）、NIfTI voxel 加载（`setMasksFromNIfTI`）。7 个 callbacks |
...
| **PanTool** | [tools/PanTool.ts](annotator-frontend/src/ts/Utils/segmentation/tools/PanTool.ts) | 右键拖拽平移画布 |
...
interface PanCallbacks {
  // getPanelOffset / setPanelOffset 已删除 — panel 偏移逻辑已内化为直接读写 canvas.offsetLeft/Top

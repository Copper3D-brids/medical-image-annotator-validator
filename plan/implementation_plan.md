...
> [!NOTE]
> Phase 5 已完成。47 个 CrosshairTool 单元测试全部通过。
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│  Level 1: GUI Tool Selection (持久化，从 UI 按钮设置)                     │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────┐ │
│  │  Pencil  │ │  Brush   │ │  Eraser  │ │  Sphere  │ │  Calculator  │ │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────────┘ │
├─────────────────────────────────────────────────────────────────────────┤
│  Level 2: Interaction State (临时修饰键 + 开关)                          │
│  ┌────────────────┐ ┌──────────────────┐ ┌───────────────────────────┐ │
│  │ Crosshair: on/off │ │ Shift: down/up │ │ Ctrl: down/up            │ │
│  └────────────────┘ └──────────────────┘ └───────────────────────────┘ │
│  ┌──────────────────┐ ┌──────────────────┐                             │
│  │ Left btn: down/up │ │ Right btn: down/up │                          │
│  └──────────────────┘ └──────────────────┘                             │
└─────────────────────────────────────────────────────────────────────────┘
```
...
#### 规则 1: Drawing Tools (GuiTool = pencil | brush | eraser)

| Crosshair | Shift | Ctrl | Left | Right | 允许的交互 | 禁止的交互 |
|-----------|-------|------|------|-------|-----------|-----------|
| OFF | UP | UP | UP | UP | zoom(wheel), sliceChange(wheel/left-drag), **arrowSlice(↑↓)**, S(toggle crosshair), Ctrl(contrast), undoRedo | draw, sphere, calculator |
| OFF | UP | UP | UP | DOWN | pan, **arrowSlice(↑↓)** | 其他全部 |
| OFF | UP | UP | DOWN | UP | sliceChange(left-drag), **arrowSlice(↑↓)** | draw, zoom, pan, crosshair, contrast |
| OFF | DOWN | UP | UP | — | (等待鼠标按下) | zoom, pan, sliceChange, **arrowSlice**, crosshair, contrast |
| OFF | DOWN | UP | DOWN | — | **draw ONLY** (pencil/brush/eraser) | 其他全部 (含 arrowSlice) |
| OFF | UP | DOWN | UP | — | **arrowSlice(↑↓)**, (等待鼠标按下) | draw, zoom, sliceChange, crosshair |
| OFF | UP | DOWN | DOWN | — | **contrast**, **arrowSlice(↑↓)** | 其他全部 |
| **ON** | — | — | UP | UP | S(toggle off), **arrowSlice(↑↓)**, undoRedo | draw, zoom, pan, sliceChange, contrast, sphere, calculator |
| **ON** | — | — | UP | DOWN | pan, **arrowSlice(↑↓)** | 其他全部 (除 S toggle off) |
| **ON** | — | — | DOWN | — | **crosshair click**, **arrowSlice(↑↓)** | 其他全部 |

> **关键**: Crosshair ON 时，Shift 和 Ctrl 按键被忽略（不触发 draw 或 contrast）。
> **arrowSlice 规则**: Arrow ↑/↓ 键切换 slice 几乎在所有状态下可用，**仅在 Shift 按下时被禁止**。

#### 规则 2: Sphere (GuiTool = sphere)

| Left | Right | 允许的交互 | 禁止的交互 |
|------|-------|-----------|-----------|
| UP | UP | zoom(wheel), sliceChange, **arrowSlice(↑↓)**, undoRedo | draw, crosshair, contrast |
| UP | DOWN | pan, **arrowSlice(↑↓)** | 其他全部 |
| **DOWN** | — | **spherePlace + sphereWheel ONLY** | zoom, pan, sliceChange, **arrowSlice**, draw, crosshair, contrast |

> **关键**: Sphere 模式下完全禁止 Crosshair、Drawing、Contrast。Shift/Ctrl/S 键被忽略。
> **arrowSlice**: Sphere 左键按下时禁止 arrowSlice，左键未按下时允许。

#### 规则 3: Calculator (GuiTool = calculator)

| Left | Right | 允许的交互 | 禁止的交互 |
|------|-------|-----------|-----------|
| UP | UP | zoom(wheel), sliceChange, **arrowSlice(↑↓)**, undoRedo | draw, crosshair, contrast |
| UP | DOWN | pan, **arrowSlice(↑↓)** | 其他全部 |
| **DOWN** | — | **calcPlace**, **arrowSlice(↑↓)** | zoom, pan, sliceChange, draw, crosshair, contrast |

> **关键**: Calculator 与 Sphere 类似，但 **左键按下时仍允许 arrowSlice**（Calculator 放置是瞬时操作，不像 Sphere 需要滚轮调半径）。

#### arrowSlice 汇总规则

> [!TIP]
> `arrowSlice` (Arrow ↑/↓ 键切换 slice) 拥有最宽松的可用性，仅 **2 种情况** 被禁止：
>
> | 被禁止的条件 | 原因 |
> |-------------|------|
> | **Shift 按下** | 正在绘画/准备绘画，防止误切换 slice |
> | **Sphere + 左键按下** | 正在放置球体 + 调节半径，防止干扰 |
>
> 其他所有状态（含 Crosshair ON、Contrast、Calculator 放置中）均可使用。
...
| 事件 | 条件 | 动作 | 被阻止的条件 |
|------|------|------|-------------|
| **S 键 pressed** | guiTool ∈ {pencil, brush, eraser} AND !shiftHeld AND !ctrlHeld | toggle crosshairEnabled | guiTool ∈ {sphere, calculator}; shiftHeld; ctrlHeld |
| **Shift down** | !crosshairEnabled AND guiTool ∈ {pencil, brush, eraser} | shiftHeld = true | crosshairEnabled; guiTool ∈ {sphere, calculator}; ctrlHeld |
| **Shift up** | shiftHeld = true | shiftHeld = false; 触发 pointerUp 结算 | — |
| **Ctrl down** | !crosshairEnabled AND guiTool ∈ {pencil, brush, eraser} AND !shiftHeld | ctrlHeld = true; 进入 contrast 模式 | crosshairEnabled; guiTool ∈ {sphere, calculator}; shiftHeld |
| **Ctrl up** | ctrlHeld = true | ctrlHeld = false; 退出 contrast 模式 | — |
| **Ctrl+Z / Ctrl+Y** | !shiftHeld AND !leftButtonDown | 执行 undo/redo | shiftHeld; 正在绘画/放置中 |
| **Arrow ↑/↓** | !shiftHeld AND !(guiTool=sphere AND leftButtonDown) | 切换 slice (±1) | shiftHeld; sphere+leftButtonDown |

#### GUI 工具切换

| 事件 | 动作 |
|------|------|
| **切换到 Sphere** | crosshairEnabled = false; shiftHeld = false; ctrlHeld = false; 调用当前工具 deactivate(); 调用 sphere.activate() |
| **切换到 Calculator** | 同上 |
| **切换到 Pencil/Brush/Eraser** | 调用当前工具 deactivate(); 调用新工具 activate(); 保留 crosshairEnabled 状态 |
| **Drawing 工具间切换** (Pencil↔Brush↔Eraser) | 直接切换 drawingTool，不影响 Level 2 状态 |

#### 鼠标事件路由

```
┌────────────────────────────────────────────────────────────────┐
│                     PointerEvent Dispatcher                     │
├────────────────────────────────────────────────────────────────┤
│                                                                  │
│  pointerdown(left) →                                             │
│    if (crosshairEnabled)     → CrosshairTool.onPointerDown()    │
│    elif (shiftHeld && drawing) → DrawingTool.onPointerDown()    │
│    elif (ctrlHeld)           → ContrastTool.onPointerDown()     │
│    elif (sphere)             → SphereTool.onPointerDown()       │
│    elif (calculator)         → SphereTool.onPointerDown(calc)   │
│    else                      → DragOperator.onPointerDown()     │
│                                (slice change via left-drag)     │
│                                                                  │
│  pointerdown(right) →        → PanTool.onPointerDown()          │
│                                                                  │
│  wheel →                                                         │
│    if (sphere && leftDown)   → SphereTool.onWheel() (radius)    │
│    elif (shiftHeld)          → ignored                           │
│    elif (ctrlHeld)           → ignored                           │
│    elif (crosshairEnabled)   → ignored                           │
│    else                      → ZoomTool.onWheel() or sliceChange │
│                                                                  │
│  Arrow ↑/↓ →                                                     │
│    if (shiftHeld)            → ignored                           │
│    elif (sphere && leftDown) → ignored                           │
│    else                      → sliceChange(±1)                   │
│                                                                  │
└────────────────────────────────────────────────────────────────┘
```
...
export type InteractionType =
    ...
    | 'arrowSlice'     // Arrow ↑/↓ keyboard slice change (更宽松的规则)
...
export class ToolCoordinator {
    ...
    isDrawing(): boolean; // shiftHeld && leftButtonDown
    ...
    dispatchArrowKey(direction: 'up' | 'down'): void; // Arrow ↑/↓ slice change
...
```typescript
canUse(interaction: InteractionType): boolean {
    const isDrawingTool = ['pencil', 'brush', 'eraser'].includes(this.guiTool);

    // ===== arrowSlice: 最宽松规则，优先判断 =====
    // 仅 2 种情况禁止: Shift 按下 或 Sphere+左键按下
    ...
    if (this.crosshairEnabled) {
        if (interaction === 'crosshairToggle') return true;  // S 键关闭
    ...
    // ===== Sphere / Calculator 模式 =====
    if (this.guiTool === 'sphere' || this.guiTool === 'calculator') {
        // 禁止: draw, crosshair, contrast
        if (['draw', 'crosshairClick', 'crosshairToggle', 'contrast'].includes(interaction))
            return false;

        if (this.leftButtonDown) {
            // 左键按下: 只允许对应工具操作 (+ arrowSlice 已在上方处理)
            if (this.guiTool === 'sphere')
                return interaction === 'spherePlace' || interaction === 'sphereWheel';
            return interaction === 'calcPlace';
        }

        // 左键未按下: 允许 zoom, sliceChange, pan, undoRedo
        return ['zoom', 'sliceChange', 'pan', 'undoRedo'].includes(interaction);
    }

    // ===== Drawing Tools 模式 =====
    if (isDrawingTool) {
        // Shift 按下 (arrowSlice 已在上方被禁止)
        if (this.shiftHeld) {
            return interaction === 'draw';  // 只允许绘画
        }
        ...
        // 左键按下 (非 shift/ctrl): slice change drag
        if (this.leftButtonDown) {
        ...
        // Idle 状态: 开放大部分功能
        return ['zoom', 'sliceChange', 'crosshairToggle', 'undoRedo'].includes(interaction)
            || interaction === 'pan';
    }
...
| 类别 | 测试数 | 说明 |
|------|--------|------|
| Drawing + Shift 互斥 | ~8 | Shift 按下时只允许 draw，其他全禁 |
| Drawing + Ctrl 互斥 | ~6 | Ctrl 按下时只允许 contrast |
| Crosshair 互斥 | ~10 | Crosshair ON 时禁止所有其他功能 |
| Sphere 互斥 | ~8 | Sphere 模式禁止 draw/crosshair/contrast |
| Calculator 互斥 | ~6 | 同 Sphere 规则 |
| GUI 工具切换 | ~6 | 切换时重置 Level 2 状态 |
| 状态转换 | ~8 | 键盘/鼠标事件正确更新状态 |
| 事件路由 | ~8 | dispatchPointer/dispatchWheel 路由到正确工具 |
| arrowSlice 可用性 | ~6 | 仅 Shift 和 Sphere+左键 时禁止，其他全允许 |
| 边界情况 | ~5 | 快速切换、同时按键等 |
| **合计** | **~71** | |

### 与现有 DrawToolCore 对比

| 现有 DrawToolCore | 新 ToolCoordinator |
|-------------------|-------------------|
| 散乱布尔标志 (3 个对象) | 集中状态管理 (1 个类) |
| 隐式互斥 (if/return 分散在事件处理中) | 显式规则矩阵 (`canUse()`) |
| 工具不知道彼此存在 | ToolCoordinator 统一调度 |
| 无法查询"当前允许什么" | `getAllowed()` 返回完整允许集合 |
| 状态切换可能遗漏清理 | `setGuiTool()` 自动重置所有 Level 2 状态 |
| 无回调通知 UI | `onStateChange` 回调驱动 UI 更新 |

---

## Phase 7A: Integration (Vue 组件同步层) ✅

> [!NOTE]
> Phase 7A 已完成。SegmentationManager 和 StateManager 已创建，提供统一入口和类型安全的状态管理。
> **当前模式**: one-way sync (NrrdTools → SegmentationManager)，SegmentationManager 尚未接管事件路由。
...
### 7B.1 Canvas Event Migration

**目标**: 将 `LeftPanelCore.vue` 中的 pointer/keyboard 事件从 NrrdTools 路由到 SegmentationManager。

```typescript
// Before (NrrdTools handles events internally)
nrrdTools.bindEvents(container);

// After (SegmentationManager handles events)
container.addEventListener('pointerdown', (e) => {
  segmentationManager.dispatchPointerDown(e);
});
container.addEventListener('pointermove', (e) => {
  segmentationManager.dispatchPointerMove(e);
});
container.addEventListener('pointerup', (e) => {
  segmentationManager.dispatchPointerUp(e);
});
container.addEventListener('wheel', (e) => {
  segmentationManager.dispatchWheel(e);
});
```
...
window.addEventListener('keydown', (e) => {
  if (e.key === 'Shift') segmentationManager.onShiftChange(true);
  if (e.key === 'Control') segmentationManager.onCtrlChange(true);
  if (e.key === 's') segmentationManager.onCrosshairToggle();
  if (e.key === 'ArrowUp') segmentationManager.dispatchArrowKey('up');
  if (e.key === 'ArrowDown') segmentationManager.dispatchArrowKey('down');
});
```

### 7B.2 Tool Adapter Implementation

每个工具需要 Adapter 来与项目层交互（Canvas 定位、NRRD 渲染等）:
...
// 项目层实现
panTool.setAdapter({
  getCanvasLeft: () => container.style.left,
  getCanvasTop: () => container.style.top,
...
interface ContrastAdapter {
  ...
  setWindowCenter(value: number): void;
  setWindowWidth(value: number): void;
  refreshDisplay(): void;  // 触发 NRRD slice 重新渲染
}
```

**SphereAdapter / CrosshairAdapter**:
```typescript
interface SphereAdapter {
  convertCursorPoint(from: AxisType, to: AxisType, 
                     mouseX: number, mouseY: number, 
                     sliceIndex: number): { x: number, y: number, sliceIndex: number } | null;
  getMaxSlice(axis: AxisType): number;
  onSpherePlaced?(origin: SphereOrigin, radius: number): void;
  onCalculatorPositionsUpdated?(positions: Map<SphereType, {...}>, currentAxis: AxisType): void;
}
```

### 7B.3 Drawing Event Takeover

**绘图预览迁移**:
- Pencil 红色轮廓: `PencilTool.onPointerMove()` → `RenderingAdapter.getDrawingContext()` 绘制
- Brush 圆形光标: `BrushTool.onPointerMove()` → 绘制圆形预览
- Eraser 虚线圆: `EraserTool.onPointerMove()` → 绘制虚线圆预览

**数据写入**:
- `onPointerUp()` 返回 `Delta[]`
- 调用 `undoManager.push(deltas)`
- 更新 `MaskLayer` 的 `Uint8Array` 数据

### 7B.4 Mask Rendering Takeover

**Canvas 层级**:
```
┌─────────────────────────────────────────────────────────┐
│ Layer 3: maskDisplayCanvas (SegmentationManager 控制)   │
│   - MaskRenderer.render() 读取 LayerManager 数据       │
│   - 根据 VisibilityManager 控制显示/隐藏             │
├─────────────────────────────────────────────────────────┤
│ Layer 2: drawingCanvas (SegmentationManager 控制)       │
│   - 工具预览 (Pencil 轮廓、Brush 光标、Crosshair)        │
├─────────────────────────────────────────────────────────┤
│ Layer 1: displayCanvas (NrrdTools 保留)                 │
│   - NRRD slice 渲染 (医学图像显示层)                    │
└─────────────────────────────────────────────────────────┘
```
...
```typescript
// 绘图完成后
const deltas = pencilTool.onPointerUp(e);
undoManager.push(deltas);
segmentationManager.render();  // 调用 MaskRenderer.render()
```

### 7B.5 State Synchronization (双向)

**Phase 7A 模式**: one-way sync
```
UI → NrrdTools → SegmentationManager (接收同步)
```

**Phase 7B 模式**: SegmentationManager 驱动
```
UI → SegmentationManager → (可选) NrrdTools (保留 NRRD 显示)
                         → MaskRenderer (Mask 显示)
                         → Tools (工具执行)
```
...
| 功能类别 | 验证项 | 预期行为 |
|---------|--------|---------|
| **Drawing** | Pencil | 画闭合区域 → 自动填充多边形内区域 |
| | Brush | 涂抹 → 连续圆形叠加 |
| | Eraser | 擦除 → 仅擦除当前 layer |
| | 坐标转换 | 缩放后画笔坐标正确 (sizeFactor) |
| **Navigation** | Pan | 右键拖动平移画布 |
| | Zoom | 滚轮定点缩放 (1x-8x) |
| | Slice Change | 滚轮/左键拖动/Arrow ↑↓ 切换 slice |
| | Contrast | Ctrl + 拖动调节 window center/width |
| **Crosshair** | Toggle | S 键启用/禁用 |
| | Click | 在视图点击后显示十字虚线 |
| | Cross-View | 切换视图时位置正确同步 |
| **Sphere** | Place | 左键点击放置球心 |
| | Radius | 滚轮调节半径 [1, 50] |
| | 3D Render | 球体跨多个 slice 正确显示 |
| **Data** | Save | 增量保存到后端 (Delta API) |
| | Load | 从后端加载 NIfTI 数据 |
| **Undo** | Undo | Ctrl+Z 撤销当前 layer 操作 |
| | Redo | Ctrl+Y 重做 |
| | Per-Layer | 切换 layer 后 undo/redo 栈独立 |
| **Coordination** | 互斥规则 | ToolCoordinator `canUse()` 行为正确 |
| | 状态转换 | Shift/Ctrl 按键正确切换模式 |

### 里程碑

**Phase 7C 完成后**:
- ✅ SegmentationManager 接管所有 Mask 相关操作
- ⏳ 需要 Phase 7D 定义与 NrrdTools 的协作
...
> - **NrrdTools**: 保留负责 NRRD slice 显示、Slice 导航、Zoom/Pan
> - 两者通过事件和回调协同工作
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                         事件路由层 (LeftPanelCore.vue)                   │
├─────────────────────────────────────────────────────────────────────────┤
│                                    ↓                                     │
│    ToolCoordinator.canUse() 检查事件类型                                  │
│                                    ↓                                     │
│    ┌─────────────────────────┬─────────────────────────────────────┐     │
│    │    SegmentationManager  │         NrrdTools/DragOperator      │     │
│    ├─────────────────────────┼─────────────────────────────────────┤     │
│    │ Mask 绘制 (Shift+左键)  │ NRRD slice 渲染                     │     │
│    │ Mask 渲染 (MaskRenderer)│ Slice 导航 (左键拖动/滚轮/Arrow)   │     │
│    │ Layer/Channel 管理      │ Zoom/Pan (滚轮/右键拖动)            │     │
│    │ Crosshair/Sphere 操作   │ Contrast 调节 (Ctrl+拖动)          │     │
│    │ Undo/Redo               │                                     │     │
│    └─────────────────────────┴─────────────────────────────────────┘     │
│                                    ↓                                     │
│                           共用 Canvas 容器                               │
│    ┌─────────────────────────────────────────────────────────────────┐   │
│    │ maskDisplayCanvas (z:3) │ drawingCanvas (z:2) │ displayCanvas (z:1)││
│    │   Mask 显示层           │   工具预览层        │   NRRD 显示层    │   │
│    └─────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────┘
```
...
**SliceChangeCallback** - NrrdTools → SegmentationManager:
```typescript
...
// NrrdTools 调用
this.sliceChangeCallback?.onSliceChange(this.currentAxis, newIndex);

// SegmentationManager 响应
onSliceChange(axis, index) {
  this.maskRenderer.render(axis, index);
}
```

**ZoomChangeCallback** - NrrdTools → SegmentationManager:
```typescript
...
// NrrdTools 调用
this.zoomChangeCallback?.onZoomChange(newSizeFactor);

// SegmentationManager 响应
onZoomChange(factor) {
  this.maskRenderer.setSizeFactor(factor);
...
// LeftPanelCore.vue
container.addEventListener('pointerdown', (e) => {
  if (segmentationManager.canHandle(e)) {
    segmentationManager.dispatchPointerDown(e);
  } else {
    // 传递给 NrrdTools/DragOperator
    nrrdTools.handlePointerDown(e);
  }
});

// SegmentationManager.canHandle()
canHandle(e: PointerEvent): boolean {
  const coord = this.toolCoordinator;
  // 绘图事件: Shift + 左键
  if (coord.isShiftHeld() && e.button === 0) return true;
  // Crosshair 事件
  if (coord.isCrosshairEnabled()) return true;
  // Sphere/Calculator 事件
  if (coord.getGuiTool() === 'sphere' || coord.getGuiTool() === 'calculator') 
    return e.button === 0;
  return false;
}
```
...
## Phase 8: Testing (Vitest) ✅ (289 tests)
...
│   └── index.ts              [NEW] ✅ Phase 2 (updated Phase 3, Phase 4, Phase 5)
├── tools/
...
├── SegmentationManager.ts    [NEW] Phase 7
├── __tests__/
│   ├── core.test.ts          [NEW] ✅ Phase 2 (46 tests)
...
> [!NOTE]
> 以下是 Phase 9 清理阶段的文件处理决策

#### 确定删除

| 文件 | 处理方式 | 理由 |
|-----|---------|------|
| `CommToolsData.ts` | ❌ 删除 | 状态已迁移到 SegmentationManager |
| `DrawToolCore.ts` | ❌ 删除 | 功能已拆分到 tools/ |
| `NrrdTools.ts` | ❌ 删除 | 替换为 `SegmentationManager.ts` |
| `coreTools/coreType.ts` | ❌ 删除 | 类型已迁移到 `core/types.ts` |

#### 部分保留 / 评估

| 文件 | 处理方式 | 详细说明 |
|-----|---------|---------|
| `DragOperator.ts` | 🔶 部分保留 | `drag()`, `drawDragSlice()` 与 NRRD 渲染耦合需保留；`drawMaskToLabelCtx()` 已被 MaskRenderer 替代可删除 |
| `coreTools/gui.ts` | 🔶 暂保留 | dat.gui 控制面板仍在使用，待 Vue 组件完全取代后再评估 |
| `coreTools/divControlTools.ts` | ✅ 保留 | slice 编号 div 辅助功能 |
| `coreTools/archive.ts` | ❓ 待确认 | 需确认是否仍在使用 |

#### DragOperator.ts 功能迁移矩阵
...
4. **Phase 6**: 验证 ToolCoordinator 互斥规则 (~71 tests)
5. **Phase 7**: 验证 getMask/setMask 与后端兼容
6. **集成测试**: 验证所有 Vue 组件与新 API 正常工作
